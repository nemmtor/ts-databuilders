...
import * as ts from '../lib/typescript-2';
...
export class TypeNodeDefaultValueGenerator extends Effect.Service<TypeNodeDefaultValueGenerator>()(
  '@TSDataBuilders/TypeNodeDefaultValueGenerator',
  {
    effect: Effect.gen(function* () {
      const { defaults } = yield* Configuration.Configuration;
      ...
      > = Effect.fnUntraced(function* (typeNode, opts) {
        return yield* Match.value(typeNode).pipe(
          ...
          Match.when(
            { kind: 'TypeReference' },
            ({ referenceName, inlineType }) =>
              Option.match(inlineType, {
                onNone: () => {
                  if (!ts.isBuiltinOpaqueType(referenceName)) {
                    return new TypeNodeDefaultValueGeneratorError.MissingBuiltinTypeDefaultError(
                      { referenceName },
                    );
                  }
                  return Effect.succeed(BUILTIN_TYPE_DEFAULTS[referenceName]);
                },
