...
type Opts = {
  ...
  visitTupleType?: TupleTypeVisitor['Type']['visit'];
...
export const visitSyntaxList = Effect.succeed<Declaration[]>([]);
...
export const visitTupleType = Effect.succeed<TupleType>({
...
export const createTestVisitorLayer = (opts?: Opts): typeof VisitorLayer =>
  Layer.mergeAll(
    ...
    Layer.succeed(
      TupleTypeVisitor,
      TupleTypeVisitor.of({
        visit: opts?.visitTupleType ?? vi.fn(() => visitTupleType),
      }),
    ),
    Layer.succeed(
      TypeNodeVisitor,
